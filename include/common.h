 ...
 *  @details  例如   host = www.bupt.edu.cn转化为 host = @verbatim\x1www\x4bupt\x3edu\x2cn\x0 @endverbatim
 ...
 * 
 *  返回值： 
 *  @return 这一条DNS answer的长度（指字符串中，可以用来计算下一个answer开始的位置）
 */
...
/**
 *  @brief 读取全部dns questions。目前不可能有超过1的question。如果有的话就是格式错误。
 ...
 * @return 写入字节数
 */
#define write_query_header_auto(x) write_dns_header(x,-1,FLAG_QUERY,1,0,0,0)
 ...
 * @return 写入字节数
 */
#define write_query_header(x,id) write_dns_header(x,id,FLAG_QUERY,1,0,0,0)
 ...
 * @return 写入字节数
 */
int write_dns_answer(char* buffer,const char*name,int type,int class,int ttl,const void*rdata);//写answer
